...
test('fires once after stabilityMins of continuous exceedance (once mode)', () => {
  ...
  const t0 = 1000 * H;
  d.onSample(0, t0);
  d.onSample(80, t0 + 60000);      // start
  d.onSample(90, t0 + 2 * 60000);
  assert.equal(d.takeEvent(t0 + 4 * 60000), null, 'before stability');
  assert.equal(d.takeEvent(t0 + 5 * 60000 + 1), null, 'event only after onSample confirmed stability');
  assert.deepEqual(d.takeEvent(t0 + 5 * 60000), null);
  d.onSample(90, t0 + 5 * 60000);  // sample at 5min: now stable
  const ev = d.takeEvent(t0 + 5 * 60000);
  assert.ok(ev && typeof ev.ts === 'number', 'fires once stability reached');
});

test('equal to threshold is NOT an exceedance (strict >)', () => {
  const d = createOverConsumeDetector({ threshold: 60, stabilityMs: 60000, oncePerOutage: true });
  const t0 = 1000 * H;
  d.onSample(60, t0);
  d.onSample(60, t0 + 60000);
  assert.equal(d.takeEvent(t0 + 60000), null);
  d.onSample(61, t0 + 2 * 60000);
  assert.ok(d.takeEvent(t0 + 2 * 60000), '61 > 60 starts the window');
});
