...
export function registerRoutes(ctx) {
  ...
  route('POST', '/api/test-notification', async (req, res) => {
    const results = await sendNotification('Test', 'Strum notification test at ' + new Date().toLocaleString());
  ...
  function _runManualSleep(ms) { return new Promise(r => setTimeout(r, ms)); }

  async function _runManualActions(actions, sceneName, results) {
    for (const action of actions) {
      try {
        if (action.type === 'delay') {
          if (action.duration > 0) {
            pushSceneTrace(sceneName, 'delay (manual)', action.duration + 'min');
            await _runManualSleep(action.duration * 60000);
          }
          results.push({ ok: true, action: 'delay:' + action.duration + 'min' });
        } else if (action.type === 'if') {
          pushSceneTrace(sceneName, 'if (manual)', 'executing both branches');
          if (action.then) await _runManualActions(action.then, sceneName, results);
          if (action.else) await _runManualActions(action.else, sceneName, results);
        } else if (action.type === 'choose') {
          pushSceneTrace(sceneName, 'choose (manual)', 'executing all choices');
          for (const choice of action.choices || []) {
            if (choice.then) await _runManualActions(choice.then, sceneName, results);
          }
          if (action.default) await _runManualActions(action.default, sceneName, results);
        } else if (action.type === 'tuya' || !action.type) {
          await controlDevice(action.device, action.value);
          pushNotification('Automation "' + sceneName + '" (manual)', deviceName(action.device) + ' = ' + (action.value ? 'ON' : 'OFF'), 'info');
          pushSceneTrace(sceneName, 'apply (manual)', action.device + '=' + (action.value ? 'ON' : 'OFF'));
          results.push({ ok: true, action: action.device || 'tuya' });
        } else if (action.type === 'notify') {
          pushNotification('Automation "' + sceneName + '" (manual)', action.message || 'Rule triggered', 'info');
          pushSceneTrace(sceneName, 'notify (manual)', action.message || '');
          results.push({ ok: true, action: 'notify' });
        } else if (action.type === 'webhook') {
          const url = action.url;
          if (url) {
            fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scene: sceneName, triggered: true, ts: Date.now() }) }).catch(() => {});
            pushSceneTrace(sceneName, 'webhook (manual)', url);
            results.push({ ok: true, action: 'webhook' });
          }
        } else if (action.type === 'priority') {
          pushNotification('Automation "' + sceneName + '" (manual)', 'Set priority: ' + action.value, 'info');
          pushSceneTrace(sceneName, 'priority (manual)', action.value);
          results.push({ ok: true, action: 'priority:' + action.value });
        }
      } catch (err) {
        pushNotification('Automation "' + sceneName + '" (manual)', 'Failed: ' + err.message, 'error');
        pushSceneTrace(sceneName, 'apply:error (manual)', err.message);
        results.push({ ok: false, action: action.device || action.type || 'unknown', error: err.message });
      }
    }
  }
  ...
  route('POST', '/api/scenes/:name/run', async (req, res) => {
    ...
    if (!scene) return sendJson(res, 404, { success: false, message: 'Scene not found' });
    const results = [];
    await _runManualActions(scene.then && scene.then.actions || [], scene.name, results);
    sendJson(res, 200, { success: true, results });
  });
