  ...
  async function checkScenes() {
    ...
    try {
      ...
      for (const scene of scenes) {
        ...
        // Scene mode: single (don't start if previous still running) / restart (default, parallel-compatible)
        const mode = scene.mode || 'restart';
        if (mode === 'single' && _sceneExecutionPromises[scene.name]) continue;

        // Oneshot scenes (grid_restored, inverter_offline) are fire-and-forget — never auto-revert
        const isOneshot = (scene.if?.conditions || []).some(c => ONESHOT_COND_TYPES.has(c.type));
        ...
        const execPromise = Promise.allSettled(scene.then.actions.map(async (action) => {
          ...
          // Duration expiry — checked regardless of conditionsMet
          if (state.active && hasDuration && now - state.appliedAt >= action.duration * 60000) {
            try {
          ...
          if (conditionsMet) {
            ...
            if (isOneshot) {
              // Oneshot scenes never auto-revert on condition change
              // Only clear failure retry flag if enough time passed
              if (state._failedAt && now - state._failedAt > 300000) { state.active = false; delete state._failedAt; }
            } else {
              try {
                await execAction(false);
        ...
        // Track scene execution promise for mode support
        _sceneExecutionPromises[scene.name] = execPromise;
        execPromise.finally(() => {
          if (_sceneExecutionPromises[scene.name] === execPromise) {
            delete _sceneExecutionPromises[scene.name];
          }
        });
      }
