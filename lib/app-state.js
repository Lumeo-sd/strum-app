...
export function createAppState(DATA_DIR, loadConfig, saveConfig, decryptSecret, pushNotification) {
  ...
  async function _executeSceneAction(scene, action, conditionsMet, ctx, isNested, runId) {
    ...
    if (isNested) {
      if (conditionsMet) await execSimple(true);
      return;
    }
    ...
    function _notify(title, msg, level) { if (!isNested) pushNotification(title, msg, level); }
    async function execSimple(apply) {
      if (action.type === 'tuya' || !action.type) {
        if (apply) {
          ...
          _notify('Automation "' + scene.name + '"', deviceName(action.device) + ' = ' + (action.value ? 'ON' : 'OFF'), 'info');
          ...
          _notify('Automation "' + scene.name + '"', deviceName(action.device) + ' reverted', 'info');
          pushSceneTrace(scene.name, 'revert', action.device);
        }
      } else if (action.type === 'notify' && apply) {
        const t = action.title || ('Automation "' + scene.name + '"');
        let m = action.message || 'Rule triggered';
        m = expandNotifyTemplate(m);
        _notify(t, m, action.critical ? 'error' : 'info');
        ...
        _notify('Automation "' + scene.name + '"', 'Priority set: ' + action.value, 'info');
