...
export function createAppState(DATA_DIR, loadConfig, saveConfig, decryptSecret, pushNotification, sendChannel) {
  ...
  function expandNotifyTemplate(message) {
    ...
    if (!_lastOutageReport) return message.replace(/\{\{[^}]+\}\}/g, '—');
    return message
  ...
  async function checkScenes(domainHint) {
    ...
    try {
      ...
      const ctx = { inverterData, tuyaDevices, healthCache: _healthCache, gridIsDown, lastGridWasDown: _lastGridWasDown, inverterConsecutiveFails: _inverterConsecutiveFails, now };
      ...
      for (const scene of scenes) {
        ...
        const conditionsMet = condRoot ? evaluateCondition(condRoot, ctx) : false;

        const execPromise = (async () => {
          try {
            for (const action of scene.then.actions) {
              if (runId.stop) break;
              await _executeSceneAction(scene, action, conditionsMet, { ...ctx, isOneshot }, false, runId);
            }
          } catch (err) {
            if (err instanceof StopExecution) {
              pushSceneTrace(scene.name, 'stop', 'execution stopped');
