          ...
          if (conditionsMet) {
            if (state.active) {
              if (hasDuration && now - state.appliedAt >= action.duration * 60000) {
                ...
                if (state._failedAt && now - state._failedAt > 300000) { state.active = false; delete state._failedAt; } log.event({ type: 'scene_skip', scene: scene.name, action: action.device || action.type, reason: 'already_active', conditionsMet: true });
              }
            } else {
