...
export function createNotifications(DATA_DIR, loadConfig, onNotify) {
  ...
  function _deduped(key, now) {
    if (_cooldown.has(key)) {
      if (now - _cooldown.get(key) < COOLDOWN_MS) return true;
      _cooldown.delete(key);
    }
    _cooldown.set(key, now);
    if (_cooldown.size > COOLDOWN_MAX_KEYS) {
      _cooldown.delete(_cooldown.keys().next().value);
    }
    return false;
  }
  ...
  function pushNotification(title, message, type, _now) {
    const now = typeof _now === 'function' ? _now() : Date.now();
    if (_deduped((type || 'info') + '|' + title + '|' + message, now)) return null;
    const id = ++_notifId;
    _notifHistory.push({ id, title, message, type: type || 'info', time: now, dismissed: false, read: false });
