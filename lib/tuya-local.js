...
export function overlayPending(pendingUpdates, now) {
  const out = {};
...
export function confirmPending(pendingUpdates, dps, now) {
  for (const [id, p] of Object.entries(pendingUpdates)) {
...
export function getLocalDevice(device) {
  ...
  function pendingOverlay() {
    return overlayPending(pendingUpdates, Date.now());
  }
  ...
  function confirmPendingLocal(dps) {
    confirmPending(pendingUpdates, dps, Date.now());
  }
    ...
    while (pushBuf && pushBuf.length >= 22) {
      ...
      try {
        ...
        if (parsed && parsed.dps) {
          ...
          confirmPendingLocal(parsed.dps);
...
export { buildFrame, parseFrame, parseDpsFromPayload };
