...
function parseFrame(data, key) {
  ...
  return { seqno, cmd, retcode, payload: plaintext.subarray(4), totalLength: expectedTotal };
}
...
export function getLocalDevice(device) {
  ...
  function onPushData(chunk) {
    ...
    while (pushBuf && pushBuf.length >= 22) {
      const frame = parseFrame(pushBuf, sessionKey);
      if (!frame) break;
  ...
  async function keeperLoop() {
    ...
    while (running) {
      try {
        ...
        log.debug(`keeper ${device.id}: ${err.message}, backoff ${backoff}ms`);
      ...
      return enqueue(async () => {
        ...
        for (let attempt = 1; attempt <= CONNECT_RETRIES; attempt++) {
          try {
            ...
            stateCache.dps[dpId] = value;
            stateCache.updatedAt = Date.now();
            log.debug(`setDP ${device.id}: dp${dpId}=${value} sent`);
            return true;
          } catch (err) {
            closeSock("control_retry_" + attempt);
            log.debug(`setDP ${device.id}: attempt ${attempt}/${CONNECT_RETRIES} failed: ${err.message}`);
