...
export function createAuth(DATA_DIR, config) {
  ...
  async function ensureAuth() {
    try {
      if (!fs.existsSync(AUTH_FILE)) {
        ...
        await atomicWriteJson(AUTH_FILE, { username: 'admin', salt, hash, mustChangePassword: true });
  ...
  function clearSessions() { for (const k of Object.keys(sessions)) delete sessions[k]; saveSessions(); }
